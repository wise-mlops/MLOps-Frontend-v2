<template>
  <div>
    <UCard class="w-64 min-h-10"
      :ui="{ rounded: 'rounded-lg rounded-lg', header: { background: 'bg-orange-100 dark:bg-orange-900 rounded-t-lg', padding: 'py-2 px-2 sm:px-4 sm:pr-2' } }">
      <template #header>
      ...
      <div class="min-h-5">
        ...
        <div class="">
          <UChip v-if="node.node.data.state" size="md" inset :color="getColor(node.node.data.state)">
            <UAvatar icon="i-heroicons-adjustments-horizontal" size="md" />
          </UChip>
        </div>
      </div>
    </UCard>
